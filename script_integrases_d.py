...
doc1 = pd.read_csv("integrases_top_hits.txt", sep='\s+', header=None)
...
#Write Result
resultado.to_csv("result_integrases.txt", index=False)

#GroupBy["CL-doc1", "k99-doc2"] and sum
resultado_suma=resultado.groupby(["CL-doc1", "k99-doc2"]).sum()
resultado_suma.to_csv("result-sum_integrases.txt")

#Calculate normalization file by number of contigs per cluster
doc1_norm = pd.read_csv("integrases_top_hits.txt", sep='\s+', header=None)
...
join_norm.to_csv("result_CLXXXXOcurrences_integrases.txt", index=False)
